...
public class Rendering {
	...
	public class RenderingMapper extends Mapper<Object, Text, Text, Text>{
		...
		public Vec3D rayTrace(Ray ray, double weight)
		{
			if(weight < 1e-5)
				return Vec3D.black;
			Vec3D ret = Vec3D.black;
			ObjectInScene obj =	 scene.findObj(ray);
			if(obj == null)
				return ret;
			Vec3D nStart = Vec3D.add(ray.start, Vec3D.mul(obj.intersect(ray), ray.direction));
			...
			double k = refractive==null?obj.reflect+obj.refractive:obj.reflect;
